      ...
      <header className="flex items-center justify-between whitespace-nowrap border-b border-white/10 glass-panel px-10 py-3 sticky top-0 z-50">
        ...
        <div className="flex flex-1 justify-end gap-8">
          <div className="hidden md:flex items-center gap-9">
            <a className="text-primary-dark text-sm font-bold leading-normal drop-shadow-[0_0_8px_rgba(192,132,252,0.4)]" href="/">Dashboard</a>
      ...
      <div className="flex-1 flex justify-center py-8 px-4 sm:px-6 lg:px-8">
        <div className="w-full max-w-7xl flex flex-col gap-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              <h1 className="text-white text-3xl md:text-4xl font-extrabold leading-tight tracking-tight mb-2 drop-shadow-md">AI Job Recommendations</h1>
              <p className="text-slate-400 text-lg">Curated for you based on your profile. Tap thumbs up/down to train your AI.</p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {jobs.length === 0 && (
              <div className="lg:col-span-2 glass-panel rounded-xl p-16 text-center">
                ...
                <h3 className="text-xl font-bold text-slate-400">No more recommendations</h3>
                <p className="text-slate-500 mt-2">Check back later or update your profile for new matches.</p>
              </div>
            )}

            {jobs.map((job) => (
              <div key={job.id} className="glass-panel rounded-xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] overflow-hidden group relative">
                ...
                <div className="p-6 md:p-8">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h3 className="text-lg font-bold text-white leading-snug tracking-wide">{job.title}</h3>
                        <p className="text-slate-400 text-sm mt-0.5">{job.company} • {job.location}</p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          <span className="text-xs font-bold text-green-400 bg-green-500/10 px-2.5 py-1 rounded-full border border-green-500/20 shadow-[0_0_8px_rgba(74,222,128,0.1)]">{job.matchScore}% Match</span>
                  ...
                  <p className="text-sm text-slate-400 leading-relaxed mb-4 line-clamp-2">{job.description}</p>

                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {job.techStack.slice(0, 4).map((skill, i) => (
                      <span key={i} className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium text-slate-300 bg-white/5 border border-white/10 group-hover:border-primary-dark/30 group-hover:bg-purple-500/10 transition-all">{skill}</span>
                    ))}
                    {job.techStack.length > 4 && <span className="text-xs text-slate-500 px-2 py-1">+{job.techStack.length - 4}</span>}
                  </div>

                  <div className="flex items-center justify-between pt-3 border-t border-white/5">
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleFeedback(job.id, "up")} className={`p-2 rounded-lg transition-all ${feedback[job.id] === "up" ? "text-green-400 bg-green-500/10 shadow-[0_0_10px_rgba(74,222,128,0.15)]" : "text-slate-500 hover:text-green-400 hover:bg-green-500/10"}`}>
                        <span className="material-symbols-outlined text-xl">{feedback[job.id] === "up" ? "thumb_up" : "thumb_up_off_alt"}</span>
                      </button>
                      <button onClick={() => handleFeedback(job.id, "down")} className={`p-2 rounded-lg transition-all ${feedback[job.id] === "down" ? "text-red-400 bg-red-500/10 shadow-[0_0_10px_rgba(248,113,113,0.15)]" : "text-slate-500 hover:text-red-400 hover:bg-red-500/10"}`}>
                        <span className="material-symbols-outlined text-xl">{feedback[job.id] === "down" ? "thumb_down" : "thumb_down_off_alt"}</span>
                      </button>
                      <button onClick={() => handleDismiss(job.id)} className="p-2 rounded-lg text-slate-500 hover:text-amber-400 hover:bg-amber-500/10 transition-all ml-1">
                        <span className="material-symbols-outlined text-xl">close</span>
                      </button>
                    </div>
                    <div className="flex items-center gap-3">
                      <button onClick={() => handleSaveNote(job.id)} className="text-xs text-slate-500 hover:text-primary-dark transition-colors flex items-center gap-1">
                        <span className="material-symbols-outlined text-[16px]">stylus_note</span>
                        {savedNotes[job.id] ? "Saved" : "Note"}
                      </button>
                      <a href="#" onClick={(e) => { e.preventDefault(); showToast("Opening external application page for " + job.title); }} className="px-4 py-2 rounded-lg bg-primary hover:bg-purple-500 text-white text-xs font-bold transition-all shadow-[0_0_15px_rgba(168,85,247,0.2)] hover:shadow-[0_0_25px_rgba(168,85,247,0.4)]">
                        Apply Externally
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="mt-3">
                    ...
                    <div className="flex justify-end mt-1">
                      <button onClick={() => handleSaveNote(job.id)} className="text-xs text-primary-dark hover:text-white transition-colors px-3 py-1 rounded-md hover:bg-primary/20">Save Note</button>
                    </div>
                  </div>
                </div>
              </div>
